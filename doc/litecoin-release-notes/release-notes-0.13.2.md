Granacoin Core version 0.13.2 is now available from:

  <https://download.granacoin.org/granacoin-0.13.2.1/>
...
Please report bugs using the issue tracker at github:
  ...
  <https://github.com/granacoin-project/granacoin/issues>
...
updates will be released anymore. Without security updates, using a granacoin
...
ECDSA signatures inside Granacoin transactions now use validation using
...
Previous versions of Granacoin Core had their mempool limited by checking
a transaction's fees against the node's minimum relay fee. There was no
upper bound on the size of the mempool and attackers could send a large
number of transactions paying just slighly more than the default minimum
relay fee to crash nodes with relatively low RAM. A temporary workaround
for previous versions of Granacoin Core was to raise the default minimum
relay fee.

Granacoin Core 0.13.2 will have a strict maximum size on the mempool. The
...
Granacoin Core 0.13.2 also introduces new default policy limits on the length and
...
This allows running granacoind without having to do any manual configuration.
...
Granacoin Core has a heuristic 'priority' based on coin value and age. This
calculation is used for relaying of transactions which do not pay the
minimum relay fee, and can be used as an alternative way of sorting
transactions for mined blocks. Granacoin Core will relay transactions with
...
entirely in Granacoin Core 0.13, and it is at this time undecided whether the
...
Granacoin Core automatically creates a hidden service to listen on, without
manual configuration. Granacoin Core will also use Tor automatically to connect
to other .onion nodes if the control socket can be successfully opened. This
will positively affect the number of available .onion nodes and their usage.

This new feature is enabled by default if Granacoin Core is listening, and
...
Granacoind can now (optionally) asynchronously notify clients through a
...
(or `settxfee <n>` rpc during runtime). A value of `n=0` signals Granacoin
Core to use floating fees. By default, Granacoin Core will use floating
fees.
...
(default: `0.0002` GRANA/kB).

At all times, Granacoin Core will cap fees at `-maxtxfee=<x>` (default:
0.10) GRANA.
Furthermore, Granacoin Core will never create transactions paying less than
...
`granacoin.conf`, where `N` is the number of MiB to allot for
raw block & undo data.
...
Then, to tunnel a SSL connection on 25883 to a RPC server bound on localhost on port 19334 do:

    stunnel -d 25883 -r 127.0.0.1:19334 -p stunnel.pem -P ''
...
caching. A sample config for apache2 could look like:
    ...
    <Location /granacoinrpc>
        ProxyPass http://127.0.0.1:5883/
        ProxyPassReverse http://127.0.0.1:5883/
        # optional enable digest auth
        # AuthType Digest
        # ...

        # optional bypass granacoind rpc basic auth
        # RequestHeader set Authorization "Basic <hash>"
        # get the <hash> from the shell with: base64 <<< granacoinrpc:<password>
    </Location>
...
Restarting granacoind will no longer clear out the list of banned peers; instead
...
- Adding `dbcache=100` in granacoin.conf
...
granacoin-cli: arguments privacy
------------------------------
...
For example:
    ...
    $ src/granacoin-cli -stdin walletpassphrase
    mysecretcode
    120
...
Various code modernizations have been done. The Granacoin Core code base has
...
- `granacoin-${VERSION}-arm-linux-gnueabihf.tar.gz`: Linux binaries for the most
  common 32-bit ARM architecture.
- `granacoin-${VERSION}-aarch64-linux-gnu.tar.gz`: Linux binaries for the most
...
for a new opcode in the Granacoin scripting system that in combination with
...
Granacoin transactions currently may specify a locktime indicating when
...
HD wallets are incompatible with older versions of Granacoin Core.
...
The two phases are distinct in the Granacoin-Qt GUI. During the first one,
...
- **Elimination of unwanted transaction malleability:** Segregating the witness
  ...
  malleability, which is a problem that makes programming Granacoin wallet
  software more difficult and which seriously complicates the design of smart
  contracts for Granacoin.
...
- **Weighting data based on how it affects node performance:** Some parts of
  each Granacoin block need to be stored by nodes in order to validate future
...
- **Signature covers value:** A simple improvement in the way signatures are
  ...
  of granacoins they think they are spending, and by having full nodes refuse to
  accept those signatures unless the amount of granacoins being spent is exactly
...
- **Increased security for multisig:** Granacoin addresses (both P2PKH addresses
  ...
  believed to be provided by Granacoin's choice of parameters for its Elliptic
...
- **More efficient almost-full-node security** Satoshi Nakamoto's original
  ...
  produce an accurate copy of Granacoin's current ledger (called the UTXO set),
  ...
  note that Granacoin Core does not provide an option to use this capability as
  of this 0.13.2 release.

- **Script versioning:** Segwit makes it easy for future soft forks to allow
  Granacoin users to individually opt-in to almost any change in the Granacoin
  Script language when those users receive new transactions.  Features
  currently being researched by Bitcoin and Granacoin Core contributors that may
...
segwit's start date of 1 January 2017 miners can update the Granacoin
client to Granacoin Core 0.13.2 to signal for segwit support. When a
...
Since Granacoin Core 0.10.0, nodes have defaulted to only relaying and
...
- `granacoin-${VERSION}-arm-linux-gnueabihf.tar.gz`: Linux binaries targeting
  the 32-bit ARMv7-A architecture.
- `granacoin-${VERSION}-aarch64-linux-gnu.tar.gz`: Linux binaries targeting
